...
- Description: Runs code in current file and exports .obj and .exe files to file or workspace directory,\n
               ...
               Please note that the type of terminal needed to run this natively on windows must be powershell in vscode terminal, and if webview is running it will compile in webview ONLY,
               once webview is closed it will compile again inside the native vscode terminal (might take a few seconds after running compile for the extension to register webview is closed).
